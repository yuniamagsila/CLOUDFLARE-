...
describe('useMessages', () => {
  ...
  it('loads inbox, sent, and unread count on mount', async () => {
    mockApiOk(inboxMessages); // fetchInbox
    mockApiOk(sentMessages); // fetchSent
  ...
  it('sets error when fetch fails', async () => {
    mockApiError('fetch failed'); // fetchInbox fails
    // fetchSent may also be called (Promise.all)
  ...
  it('sends a message and refreshes', async () => {
    mockApiOk(inboxMessages); // initial fetchInbox
    mockApiOk(sentMessages); // initial fetchSent
    mockApiOk(null); // insertMessage
    mockApiOk(inboxMessages); // re-fetchInbox
    mockApiOk(sentMessages); // re-fetchSent
  ...
  it('marks a message as read and decrements unread count', async () => {
    mockApiOk(inboxMessages); // initial fetchInbox
    mockApiOk(sentMessages); // initial fetchSent
    mockApiOk(null); // markMessageRead
  ...
  it('marks all messages as read', async () => {
    mockApiOk(inboxMessages); // initial fetchInbox
    mockApiOk(sentMessages); // initial fetchSent
    mockApiOk(null); // markAllMessagesRead
